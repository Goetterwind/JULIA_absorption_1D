...
% Calculation basis for the presentation at CIFE
% added aspect ratios for pump and extraction

clear all;
tic;
%constants
const.N1per = 1.388e20;
const.c = 3e8;
const.h = 6.626e-34;
...
% aspect rations for pump and extraction in the case the pump and
% extraction is not parallel
pump_aspect = 1;
extr_aspect = 1;

% redefine the crystal length for the pump ;)
crystal.length = crystal.length * pump_aspect;
...
% pump
pump.s_abs = 0.7e-20; % absorption cross section in cm² (0.778e-20 pour DA)
pump.s_ems = 0.22e-20; % emission cross section in cm²(0.195e-20 pour DA)
pump.I = 16e3; % Pump intensity in W/cm²
pump.T = 1e-3; % pump duration in s
pump.wavelength = 940e-9;% pump wavelength in m

% laser
laser.s_abs = 0.12e-20; %cm2(1.16e-21 pour DA)
laser.s_ems = 2.4e-20; %cm2(2.48e-20)
laser.I = 1e6; % laser intensity
laser.T = 1e-9; % laser duration
laser.wavelength = 1030e-9; % lasing wavelength in m

% % additional parameter for the ASE calculations if wanted
% laser.radius = 2; % radius of disc
...
% beta_crystal_ASE = zeros(steps.crys,1);
...
% gain_local_ASE = zeros(steps.crys,1);
...
% [beta_crystal_ASE,beta_store_ASE,pulse,Ntot_gradient,M_ASE_store] = beta_int_3_ASE(beta_crystal_ASE,pulse,const,crystal,steps,pump,laser,mode,Ntot_gradient);
...
beta_min = zeros(25,1);
beta_min(:) = laser.s_abs/(laser.s_abs+laser.s_ems);
...
% [beta_crystal_l,beta_store_l,pulse] = beta_int3(beta_crystal,pulse,const,crystal,steps,laser,mode);
% 
% % integration yields the energy density, this has to be multiplied with the
% % "average" surface of the pumped area
...
gain_local(:) = -(laser.s_abs - beta_crystal(:)*(laser.s_abs+laser.s_ems)).*Ntot_gradient(:);
% gain_local_ASE(:) = -(laser.s_abs - beta_crystal_ASE(:)*(laser.s_abs+laser.s_ems)).*Ntot_gradient(:);
...
%% now do the integration to get the temporal evolution

% correct the path length for extr/pump, too
grid_z = grid_z/pump_aspect*extr_aspect;

% gain_local_ASE_2(:,:) = -(laser.s_abs - beta_store_ASE(:,:)*(laser.s_abs+laser.s_ems)).*crystal.doping*const.N1per;
% gain_temp_ASE=exp(2*trapz(grid_z,gain_local_ASE_2,1));
gain_local_2(:,:) = -(laser.s_abs - beta_store(:,:)*(laser.s_abs+laser.s_ems)).*crystal.doping*const.N1per;
gain_temp=exp(2*trapz(grid_z,gain_local_2,1));

% gain in reflection with R=1
gain_pass=exp(2*trapz(grid_z,gain_local));
disp(gain_pass);

% the real M_ASE
% M_ASE_store_real=crystal.tfluo.*(1./M_ASE_store);